...
const ToastContainer = ({
  ...
  <div className="fixed top-5 right-5 z-[9999] flex flex-col gap-2.5 pointer-events-none">
    ...
    {toasts.map((t) => (
      ...
      >
        <span
          className={`shrink-0 mt-0.5 ${TOAST_ICON_STYLES[t.type]}`}
        >
          {TOAST_ICONS[t.type]}
        </span>

        <p className="text-[13.5px] font-medium leading-snug flex-1">
          {t.message}
        </p>
...
const ProductCatalog = () => {
  ...
  const fetchProducts =
    async () => {
      ...
      try {
         ...
         *
         * We filter approved products in JavaScript
         * to avoid Firestore composite index errors.
         */
        const snapshot =
          await getDocs(
            query(
  ...
  useEffect(() => {
     ...
     * IMPORTANT:
     *
     * Pending supplier submissions are now
     * stored in 'products'.
     *
     * We no longer listen to 'pendingProducts'.
     */
    const q = query(
      collection(
        db,
    ...
    const unsubscribe =
      onSnapshot(
        ...
        (error) =>
          console.error(
            'Pending products listener error:',
            error,
          ),
      );
  ...
  const handleAddProduct =
    async (e) => {
      ...
      try {
        ...
        showToast(
          'Product submitted for admin approval. You will be notified once it is reviewed.',
          'success',
        );
    ...
    async (
      ...
      const confirmed =
        await showConfirm({
          ...
          message:
            `Are you sure you want to delete "${productName}"? This will also remove it from admin inventory.`,
