...
const AdminPayments = () => {
  ...
  const fetchPayments = async () => {
    try {
      ...
      // Only show supplier-payment records in Admin Payments.
      // This hides general/customer payment records such as "Full Payment (Online)"
      // without changing the table structure, columns, or payment functionality.
      const supplierPaymentsOnly = paymentsData.filter(payment =>
        payment.paymentType === 'INITIAL' || payment.paymentType === 'FINAL'
      );

      setPayments(supplierPaymentsOnly);
    } catch (error) {
