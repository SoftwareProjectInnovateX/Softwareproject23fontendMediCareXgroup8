...
const Checkout = () => {
    ...
    const PAYMENT_GATEWAY_CONFIG = {
        ...
        NOTIFY_URL: `${import.meta.env.VITE_API_URL || import.meta.env.VITE_API_URL_RAILWAY}/api/customer-orders/notify`,
    ...
    const handlePlaceOrder = async () => {
        ...
        if (orderData.paymentMethod === 'ONLINE') {
            ...
            try {
                ...
                const response = await fetch(`${import.meta.env.VITE_API_URL_RAILWAY}/api/customer-orders`, {
                    method: 'POST',
    ...
    const processOnlinePayment = async (orderId) => {
        ...
        try {
            ...
            const hashRes = await fetch(`${import.meta.env.VITE_API_URL || import.meta.env.VITE_API_URL_RAILWAY}/api/customer-orders/generate-hash?orderId=${orderId}&amount=${PAYMENT_GATEWAY_CONFIG.TOTAL_AMOUNT}&currency=${PAYMENT_GATEWAY_CONFIG.CURRENCY}`);
    ...
    const finalizeMediCareXOrder = async (referenceId, shouldClearCart = false, isAlreadyCreated = false, currentOrderData = orderData, currentRxId = rxId) => {
        try {
            ...
            if (!isAlreadyCreated) {
                ...
                const authHeaders = await getAuthHeaders();
                const response = await fetch(`${import.meta.env.VITE_API_URL || import.meta.env.VITE_API_URL_RAILWAY}/api/customer-orders`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', ...authHeaders },
                    body: JSON.stringify({
                        ...currentOrderData,
