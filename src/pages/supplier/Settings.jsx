...
import {
  MdPerson, MdAccountBalance, MdLock,
  MdNotifications, MdInfo, MdCheckCircle, MdCancel,
} from 'react-icons/md';
...
const Settings = () => {
  ...
  const [profileData, setProfileData] = useState({
    ...
    rating: 0, status: 'active', supplierId: '', categories: [],
  });
  const [bankData, setBankData] = useState({
    bankName: '', accountHolderName: '', accountNumber: '',
    routingNumber: '', swiftCode: '', bankAddress: '', accountType: 'Checking',
  });
  const [securityData, setSecurityData] = useState({
    currentPassword: '', newPassword: '', confirmPassword: '',
  });
  const [notificationPrefs, setNotificationPrefs] = useState({
    emailNotifications: true, orderUpdates: true, lowStockAlerts: true,
    paymentNotifications: true, marketingEmails: false,
  });
  ...
  const fetchSupplierData = async () => {
    try {
      ...
      if (snap.exists()) {
        const d = snap.data();
        setProfileData({ name: d.name || '', contactPerson: d.contactPerson || '', email: d.email || '', phone: d.phone || '', rating: d.rating || 0, status: d.status || 'active', supplierId: d.supplierId || '', categories: d.categories || [] });
  ...
  const saveProfile = async (e) => {
    ...
    try {
      await updateDoc(doc(db, 'suppliers', SUPPLIER_ID), { ...profileData, updatedAt: Timestamp.now() });
      showMessage('success', 'Profile updated successfully!');
    } catch (error) {
      if (error.code === 'not-found') {
        try {
          await setDoc(doc(db, 'suppliers', SUPPLIER_ID), { ...profileData, bankDetails: {}, notifications: {}, createdAt: Timestamp.now(), updatedAt: Timestamp.now() });
          showMessage('success', 'Profile created successfully!');
        } catch (e) { showMessage('error', 'Failed to create profile: ' + e.message); }
      } else { showMessage('error', 'Failed to update profile: ' + error.message); }
  ...
  const saveBankDetails = async (e) => {
    e.preventDefault(); setSaving(true);
    try {
      await updateDoc(doc(db, 'suppliers', SUPPLIER_ID), { bankDetails: bankData, updatedAt: Timestamp.now() });
  ...
  const saveNotifications = async () => {
    setSaving(true);
    try {
      await updateDoc(doc(db, 'suppliers', SUPPLIER_ID), { notifications: notificationPrefs, updatedAt: Timestamp.now() });
      showMessage('success', 'Notification preferences updated!');
    } catch (error) { showMessage('error', 'Failed to update notifications: ' + error.message); }
    finally { setSaving(false); }
  };

  const changePassword = async (e) => {
    e.preventDefault();
    if (securityData.newPassword !== securityData.confirmPassword) { showMessage('error', 'New passwords do not match!'); return; }
    if (securityData.newPassword.length < 6) { showMessage('error', 'Password must be at least 6 characters long!'); return; }
    setSaving(true);
    try {
      showMessage('success', 'Password changed successfully!');
      setSecurityData({ currentPassword: '', newPassword: '', confirmPassword: '' });
    } catch (error) { showMessage('error', 'Failed to change password: ' + error.message); }
    finally { setSaving(false); }
  };
  ...
  /* ---- shared input class ---- */
  const inputCls = "w-full px-3.5 py-2.5 border border-slate-300 rounded-lg text-sm text-slate-800 transition-all duration-200 focus:outline-none focus:border-blue-500 focus:ring-3 focus:ring-blue-500/10 placeholder:text-slate-400";
  ...
  const notifItems = [
    ...
    { key: 'paymentNotifications',  label: 'Payment Notifications',   desc: 'Get notified about payments and invoices' },
    { key: 'marketingEmails',       label: 'Marketing Emails',        desc: 'Receive promotional offers and product updates' },
  ];
      ...
      <div className="bg-white rounded-xl p-8 shadow-sm" style={{ animation: 'fadeIn 0.3s ease-in' }}>
            ...
            <form onSubmit={saveProfile} className="max-w-[900px]">
              <div className="grid grid-cols-2 gap-5 mb-6">
                ...
                <div className="flex flex-col">
                  ...
                  <input type="text" value={profileData.supplierId} disabled className={`${inputCls} bg-slate-100 cursor-not-allowed`} />
                  <small className="text-xs text-slate-400 mt-1">Auto-generated (e.g., S001, S002...)</small>
                </div>

                {[
                  { label: 'Company Name *',  key: 'name',          type: 'text',   ph: 'e.g., Jeny Bel',      req: true },
                  { label: 'Contact Person *', key: 'contactPerson', type: 'text',   ph: 'e.g., John Silva',    req: true },
                  { label: 'Email Address *',  key: 'email',         type: 'email',  ph: 'e.g., john@medi.com', req: true },
                  { label: 'Phone Number *',   key: 'phone',         type: 'tel',    ph: 'e.g., 0760689429',    req: true },
                  { label: 'Rating',           key: 'rating',        type: 'number', ph: 'e.g., 4.5',           req: false, extra: { step: '0.1', min: '0', max: '5' } },
                ].map((f) => (
                  <div key={f.key} className="flex flex-col">
                    ...
                    <input
                      ...
                      onChange={(e) => setProfileData({ ...profileData, [f.key]: f.type === 'number' ? parseFloat(e.target.value) || 0 : e.target.value })}
                      className={inputCls}
                      {...(f.extra || {})}
                    />
                  </div>
                ...
                <div className="flex flex-col">
                  ...
                  <select required value={profileData.status} onChange={(e) => setProfileData({ ...profileData, status: e.target.value })} className={inputCls}>
                  ...
                  <div className="flex flex-col gap-2.5">
                    {profileData.categories.map((cat, i) => (
                      <div key={i} className="flex gap-2.5 items-center">
                        <select value={cat} onChange={(e) => handleCategoryChange(i, e.target.value)} className={`${inputCls} flex-1`} required>
                          <option value="">Select a category</option>
                          {AVAILABLE_CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
                        </select>
                        <button type="button" onClick={() => removeCategory(i)} className="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium rounded-lg border-none cursor-pointer transition-colors">
                          Remove
                        </button>
                      </div>
                    ))}
                    <button type="button" onClick={addCategory} className="self-start px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium rounded-lg border-none cursor-pointer transition-colors">
                      + Add Category
                    </button>
                  </div>
                  <small className="text-xs text-slate-400 mt-1">Available: Medicine, Equipments, Baby Items, Skin Care</small>
                </div>
              </div>
        ...
        {activeTab === 'bank' && (
          <div>
            ...
            <form onSubmit={saveBankDetails} className="max-w-[900px]">
              <div className="grid grid-cols-2 gap-5 mb-6">
                {[
                  { label: 'Bank Name *',             key: 'bankName',           type: 'text', ph: 'e.g., Bank of America', req: true },
                  { label: 'Account Holder Name *',   key: 'accountHolderName',  type: 'text', ph: 'e.g., MedSupply Co.',   req: true },
                  { label: 'Account Number *',        key: 'accountNumber',      type: 'text', ph: 'e.g., 1234567890',      req: true },
                  { label: 'Routing Number',          key: 'routingNumber',      type: 'text', ph: 'e.g., 021000021',       req: false },
                  { label: 'SWIFT/BIC Code',          key: 'swiftCode',          type: 'text', ph: 'e.g., BOFAUS3N',        req: false },
                ].map((f) => (
                  <div key={f.key} className="flex flex-col">
                    <label className="text-sm font-medium text-slate-700 mb-2">{f.label}</label>
                    <input type={f.type} required={f.req} placeholder={f.ph} value={bankData[f.key] || ''} onChange={(e) => setBankData({ ...bankData, [f.key]: e.target.value })} className={inputCls} />
                  </div>
                ))}
                ...
                <div className="flex flex-col">
                  ...
                  <select required value={bankData.accountType || 'Checking'} onChange={(e) => setBankData({ ...bankData, accountType: e.target.value })} className={inputCls}>
                ...
                <div className="flex flex-col col-span-2">
                  ...
                  <input type="text" placeholder="Bank branch address" value={bankData.bankAddress || ''} onChange={(e) => setBankData({ ...bankData, bankAddress: e.target.value })} className={inputCls} />
                </div>
              </div>
            ...
            <form onSubmit={changePassword} className="max-w-[900px]">
              <div className="grid grid-cols-2 gap-5 mb-6">
                <div className="flex flex-col col-span-2">
                  ...
                  <input type="password" required placeholder="Enter current password" value={securityData.currentPassword} onChange={(e) => setSecurityData({ ...securityData, currentPassword: e.target.value })} className={inputCls} />
                </div>
                {[
                  { label: 'New Password *',         key: 'newPassword',      ph: 'Enter new password' },
                  { label: 'Confirm New Password *', key: 'confirmPassword',  ph: 'Confirm new password' },
                ].map((f) => (
                  <div key={f.key} className="flex flex-col">
                    <label className="text-sm font-medium text-slate-700 mb-2">{f.label}</label>
                    <input type="password" required placeholder={f.ph} value={securityData[f.key]} onChange={(e) => setSecurityData({ ...securityData, [f.key]: e.target.value })} className={inputCls} />
                  </div>
                ))}
              </div>
              ...
              <div className="flex gap-3 pt-6 border-t border-slate-200">
                <button type="submit" disabled={saving} className="px-6 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 disabled:cursor-not-allowed text-white text-[15px] font-medium rounded-lg border-none cursor-pointer transition-colors">
        ...
        {activeTab === 'notifications' && (
          <div>
            ...
            <p className="text-sm text-slate-500 mb-8">Choose what notifications you want to receive</p>
            ...
            <div className="max-w-[900px]">
              ...
              <div className="flex gap-3 pt-6 border-t border-slate-200">
                <button onClick={saveNotifications} disabled={saving} className="px-6 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 disabled:cursor-not-allowed text-white text-[15px] font-medium rounded-lg border-none cursor-pointer transition-colors">
...
function FormActions({ saving, saveLabel, onCancel }) {
  return (
    <div className="flex gap-3 pt-6 border-t border-slate-200">
      ...
      <button type="button" onClick={onCancel} className="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[15px] font-medium rounded-lg border-none cursor-pointer transition-colors">
        Cancel
      </button>
    </div>
  );
