 ...
 *
 * KEY FIX: Modals are now conditionally MOUNTED (not just conditionally shown).
 *
 * Root cause of the "page freezes after visiting" bug:
 *   ModalWrap renders a `position:fixed inset-0` backdrop div unconditionally.
 *   The old code passed `order={showRejectModal ? orderToReject : null}` and
 *   `order={selectedOrder}` — so even when both were null/false, ModalWrap still
 *   mounted its invisible full-screen div (before the null-check inside each
 *   modal could return null). That backdrop intercepted every click on the page.
 *
 * Fix: gate both modals with {condition && <Modal />} so ModalWrap — and its
 * backdrop — only ever exist in the DOM when the modal is genuinely open.
 */

// ─── Inline confirm/alert modal (replaces window.confirm / window.alert) ─────
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          ...
          <button
            ...
            style={{
              ...
              background: alertOnly ? "#3b82f6" : "#10b981",
...
export default function PurchaseOrders() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      setAuthReady(true); // mark auth as settled regardless of outcome
    });
    return () => unsub();
  }, []);
  ...
  /* ── FETCH ────────────────────────────────────────────────────────────────── */
  /**
   * NOTE: compound queries (supplierId + status + orderBy createdAt) require a
   * composite Firestore index. Add via Firebase console or firestore.indexes.json.
   */
  ...
  // Wait for auth to settle before the first fetch so supplierId is never
  // stale-null on the initial render.
  useEffect(() => {
  ...
  /* ── APPROVE ──────────────────────────────────────────────────────────────── */
  /**
   * All six write steps are committed in a single writeBatch — atomic,
   * all-or-nothing. If any step fails the entire batch is rolled back.
   */
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      {/*
        ══ THE FIX ══════════════════════════════════════════════════════════════
        Both modals are gated with {condition && <Modal />}.

        Before this fix:
          • <OrderDetailsModal order={selectedOrder} />  — always mounted
          • <RejectOrderModal order={showRejectModal ? orderToReject : null} />  — always mounted

        ModalWrap mounts a `position:fixed inset-0` backdrop before the null-check
        inside each modal runs. Closed = invisible backdrop = all clicks swallowed.

        After this fix: ModalWrap only exists in the DOM when the modal is open.
        ═════════════════════════════════════════════════════════════════════════
      */}
      {selectedOrder && (
        <OrderDetailsModal
