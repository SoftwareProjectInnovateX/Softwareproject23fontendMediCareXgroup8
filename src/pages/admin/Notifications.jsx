...
const Notifications = () => {
  ...
  const handleOrderApproval = async (notification) => {
    ...
    try {
      setProcessing(true);
      const res = await fetch(
        `${API_BASE}/notifications/${notification.id}/mark-received`,
        {
          method: "PATCH",
        },
      );
  ...
  const getNotificationIcon = (type, size = 24) => {
    switch (type) {
      case "ORDER_PLACED":
        return <MdShoppingCart size={size} className="text-blue-500" />;
      case "ORDER_APPROVED":
        return <MdCheckCircle size={size} className="text-emerald-500" />;
      case "ORDER_REJECTED":
        return <MdCancel size={size} className="text-red-500" />;
      case "LOW_STOCK_ALERT":
        return <MdWarning size={size} className="text-amber-500" />;
      case "ACCOUNT_REQUEST":
        return <MdPersonAdd size={size} className="text-teal-500" />;
      default:
        return <MdCampaign size={size} className="text-slate-500" />;
    }
  };
  ...
  const statCards = [
    ...
    { label: "Unread", value: unreadCount, accent: "border-amber-400" },
    {
      ...
      value: notifications.filter((n) => n.type === "ORDER_APPROVED").length,
      accent: "border-emerald-400",
    },
    {
      label: "Low Stock",
      value: notifications.filter((n) => n.type === "LOW_STOCK_ALERT").length,
      ...
      <div className="flex justify-between items-start mb-7 flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-500 text-[15px]">
            Stay updated with supplier responses and stock alerts
          </p>
        </div>
        {unreadCount > 0 && (
          <button
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm py-20 text-center">
            ...
            <small className="text-sm text-slate-400">
              You'll see updates from suppliers here
            </small>
          </div>
        ) : (
          filteredNotifications.map((notification) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[15px] text-slate-800 mb-1.5 leading-relaxed m-0">
          ...
          >
            ...
            <div className="p-7">
              ...
              <div className="bg-slate-50 border-l-4 border-blue-500 px-4 py-4 rounded-lg mb-6">
                <p className="m-0 text-[15px] text-slate-800 leading-relaxed">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                {[
                  selectedNotification.poId && {
                    label: "PO ID",
                    value: selectedNotification.poId,
                  },
                  selectedNotification.supplierName && {
                    label: "Supplier",
                    value: selectedNotification.supplierName,
                  },
                  selectedNotification.productName && {
                    label: "Product",
                    value: selectedNotification.productName,
                  },
                  selectedNotification.quantity && {
                    label: "Quantity",
                    value: `${selectedNotification.quantity} units`,
                  },
                  selectedNotification.totalAmount && {
                    label: "Total Amount",
                    value: `Rs. ${Number(selectedNotification.totalAmount).toFixed(2)}`,
                  },
                ]
                  .filter(Boolean)
                  .map((item) => (
                    <div key={item.label} className="flex flex-col">
                      <label className="text-[13px] text-slate-500 font-medium mb-1.5">
                        {item.label}
                      </label>
